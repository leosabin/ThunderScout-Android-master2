...
public class AverageScoutData implements Serializable {
    ...
    public List<String> getDefensesList() {
        List<String> defensesList = new ArrayList<>();

        for (ScoutData data : dataList) {
            if (data.getDefense() !=null && !data.getDefense().isEmpty()) {
                defensesList.add(data.getDefense());
            }
        }
        return defensesList;
    }
